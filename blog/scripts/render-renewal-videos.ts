...
async function main() {
  ...
  // Start from what's already generated: acts as the accumulator, and lets a
  // re-run RESUME (skip combos already rendered) after any interruption.
  const registry: Record<string, RenewalVideoMeta> = {}
  try {
    const mod = await import(OUT + '?t=' + Date.now())
    Object.assign(registry, mod.RENEWAL_VIDEOS ?? {})
  } catch { /* first run */ }

  const writeRegistry = () => {
    const ordered = Object.fromEntries(Object.keys(registry).sort().map((k) => [k, registry[k]]))
...
`
  ...
  for (const key of combos) {
    ...
    if (!force && registry[slug]) { console.log(`• ${slug} … skip (already rendered)`); continue }
    ...
    writeRegistry() // incremental: an interruption never loses completed combos
    rendered++
    console.log('done')
  }

  console.log(`\nWrote ${OUT} (${Object.keys(registry).length} combos total, ${rendered} rendered this run)`)
}
